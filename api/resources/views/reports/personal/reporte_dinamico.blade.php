...
<head>
    ...
    <style>
        ...
        .header h2 { margin: 0; font-size: 14px; text-transform: uppercase; color: #475569; }
        .logo { position: absolute; top: 0; left: 0; width: 60px; height: 60px; }
        .info-panel { clear: both; margin-bottom: 15px; }
        .info-panel p { margin: 3px 0; font-size: 10px; color: #64748b; }
        .info-panel strong { color: #334155; }
        
        table { width: 100%; border-collapse: collapse; margin-top: 10px; }
        th { background-color: #f1f5f9; color: #334155; font-weight: bold; padding: 8px; border: 1px solid #cbd5e1; text-align: left; text-transform: uppercase; font-size: 10px; }
        td { border: 1px solid #cbd5e1; padding: 7px; color: #475569; }
...
<body>
        ...
        <tbody>
            ...
            <tr>
                ...
                @elseif($entity === 'pqrs')
                    ...
                    <td>{{ \Illuminate\Support\Str::limit($row->respuesta, 30) ?: 'Sin respuesta' }}</td>
                    <td>{{ $row->email }}</td>
                    <td>{{ $row->telefono ?? 'N/A' }}</td>
                    <td>
                        @php
    ...
    <div class="footer">
        Saluvanta EPS - Soft. Sanitech - Reporte de {{ ucfirst($entity) }} - Documento Interno y Confidencial
    </div>
</body>
</html>
