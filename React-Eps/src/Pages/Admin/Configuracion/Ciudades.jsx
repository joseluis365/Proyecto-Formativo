...
import CiudadModal from "@/components/Modals/CiudadModal";
...
export default function Ciudades() {
    ...
    useEffect(() => {
        setTitle("Ciudades");
        setSubtitle("Gestiona las ciudades y municipios registrados en el sistema.");
    }, []);
    ...
    const handleToggleStatus = async (item) => {
        if (item.id_estado === 1) {
            const result = await Swal.fire({
                ...
                text: "Esta acción marcará la ciudad como inactiva.",
                icon: "warning",
            ...
            if (result.isConfirmed) {
                try {
                    await api.delete(`/ciudades/${item.codigo_postal}`);
                    Swal.fire("Desactivada", "La ciudad ha sido desactivada.", "success");
                    fetchCiudades();
                } catch (error) {
                    Swal.fire("Error", "No se pudo desactivar la ciudad.", "error");
                }
            }
        } else {
    ...
    const columns = [
        ...
        },
        {
            ...
            header: "Departamento",
            render: (item) => <span className="text-sm font-medium text-primary">{item.departamento_nombre || "No asignado"}</span>
        },
        {
            key: "id_estado",
            header: "Estado",
