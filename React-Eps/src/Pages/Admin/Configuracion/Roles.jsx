...
export default function Roles() {
    ...
    useEffect(() => {
        setTitle("Roles");
        setSubtitle("Gestiona los roles y tipos de usuario del sistema.");
    }, []);
    ...
    const handleToggleStatus = async (item) => {
        if (item.id_estado === 1) {
            const result = await Swal.fire({
                ...
                text: "Esta acción marcará el rol como inactivo.",
                icon: "warning",
            ...
            if (result.isConfirmed) {
                try {
                    await api.delete(`/roles/${item.id_rol}`);
                    Swal.fire("Desactivado", "El rol ha sido desactivado.", "success");
                    fetchRoles();
                } catch (error) {
                    Swal.fire("Error", "No se pudo desactivar el rol.", "error");
                }
            }
        } else {
