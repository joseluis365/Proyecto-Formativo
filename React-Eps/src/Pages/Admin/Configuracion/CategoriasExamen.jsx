...
import CategoriaExamenModal from "@/components/Modals/CategoriaExamenModal";
...
export default function CategoriasExamen() {
    ...
    const {
        ...
        setStatus,
        fetchData
    } = useTableData("/categorias-examen");

    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editData, setEditData] = useState(null);

    useEffect(() => {
        setTitle("Categorías de Examen");
        setSubtitle("Gestiona las agrupaciones para los laboratorios y estudios.");
    }, []);
    ...
    const handleToggleStatus = async (item) => {
        if (item.id_estado === 1) {
            ...
            if (result.isConfirmed) {
                try {
                    await api.delete(`/categorias-examen/${item.id_categoria_examen}`);
