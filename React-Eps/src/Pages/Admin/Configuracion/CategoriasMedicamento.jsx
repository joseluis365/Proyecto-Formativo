...
import CategoriaMedicamentoModal from "@/components/Modals/CategoriaMedicamentoModal";
...
export default function CategoriasMedicamento() {
    ...
    useEffect(() => {
        setTitle("Categorías de Medicamento");
        setSubtitle("Gestiona las clasificaciones farmacéuticas del sistema.");
    }, []);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await api.get("/categorias-medicamento", {
                params: {
            ...
            console.error("Error fetching categorias de medicamento:", error);
        } finally {
            setLoading(false);
    ...
    const handleToggleStatus = async (item) => {
        if (item.id_estado === 1) {
            ...
            if (result.isConfirmed) {
                try {
                    await api.delete(`/categorias-medicamento/${item.id_categoria}`);
    ...
    const columns = [
        {
            key: "categoria",
            header: "Categoría de Medicamento",
            render: (item) => <span className="font-semibold">{item.categoria}</span>
        },
        {
            key: "id_estado",
            header: "Estado",
    ...
    return (
        <div className="bg-white dark:bg-gray-900 rounded-xl">
            <div className="flex flex-wrap justify-between items-center gap-4 mb-8">
                <PrincipalText icon="medication" text="Categorías de Medicamento" number={total} />
