...
import { especialidadSchema } from "@/schemas/especialidadSchema";
...
export default function EspecialidadModal({ isOpen, onClose, onSuccess, editData = null }) {
    ...
    const handleFormSubmit = async (data) => {
        try {
            if (isEdit) {
                await api.put(`/configuracion/especialidades/${editData.id_especialidad}`, data);
                Swal.fire("Éxito", "Especialidad actualizada correctamente", "success");
            } else {
                await api.post("/configuracion/especialidades", data);
