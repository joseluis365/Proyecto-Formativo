...
export default function SuperAdminRolModal({ isOpen, onClose, onSuccess, editData = null }) {
    ...
    const handleFormSubmit = async (data) => {
        try {
            if (isEdit) {
                await superAdminApi.put(`/roles/${editData.id_rol}`, data);
                Swal.fire("Éxito", "Rol actualizado correctamente (SuperAdmin)", "success");
            } else {
                await superAdminApi.post("/roles", data);
                Swal.fire("Éxito", "Rol creado correctamente (SuperAdmin)", "success");
            }
            onSuccess();
            onClose();
        } catch (error) {
            if (!handleApiErrors(error, setError)) {
                Swal.fire("Error", "Ocurrió un error al procesar la solicitud (SuperAdmin)", "error");
            }
        }
    };
    ...
    return (
        <BaseModal>
            <ModalHeader
                title={isEdit ? "Editar Rol (SuperAdmin)" : "Nuevo Rol (SuperAdmin)"}
                icon="group"
                onClose={onClose}
            />
            <div className="p-6">
