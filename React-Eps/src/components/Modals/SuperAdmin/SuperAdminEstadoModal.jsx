...
export default function SuperAdminEstadoModal({ isOpen, onClose, onSuccess, editData = null }) {
    ...
    const handleFormSubmit = async (data) => {
        try {
            if (isEdit) {
                await superAdminApi.put(`/estados/${editData.id_estado}`, data);
                Swal.fire({
                    icon: 'success',
                    title: 'Éxito',
                    text: 'Estado actualizado correctamente (SuperAdmin)',
                    showConfirmButton: false,
                    timer: 1500
                });
            } else {
                await superAdminApi.post("/estados", data);
                Swal.fire({
                    icon: 'success',
                    title: 'Éxito',
                    text: 'Estado creado correctamente (SuperAdmin)',
                    showConfirmButton: false,
                    timer: 1500
                });
            }
            ...
            if (error.response?.status === 403) {
                Swal.fire({
                    ...
                    text: error.response.data.message || "No tienes permiso para modificar este estado.",
                });
                return;
            }
    ...
    return (
        <BaseModal>
            <ModalHeader
                title={isEdit ? "EDITAR ESTADO (SUPERADMIN)" : "NUEVO ESTADO (SUPERADMIN)"}
                icon="settings"
                onClose={onClose}
            />
            <div className="p-6">
